...
import { ArrowLeft, CheckCircle, Clock, Package, Truck, ShieldCheck, AlertTriangle, Copy } from 'lucide-react';
...
export default function OrderTrackingClient() {
  ...
  useEffect(() => {
    const rawId = params.id;
    const id = Array.isArray(rawId) ? rawId[0] : rawId;
    if (!loading && !user && id && id !== 'placeholder') {
      router.push('/auth/login?redirect=/order/' + id);
    }
  ...
  useEffect(() => {
    const rawId = params.id;
    const id = Array.isArray(rawId) ? rawId[0] : rawId;
    if (user && id && id !== 'placeholder') fetchOrder();
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 relative overflow-hidden selection:bg-purple-500/30 transition-colors duration-300">
      ...
      <main className="container mx-auto px-6 py-10 relative z-10 max-w-5xl">
        ...
        <div className="p-8 rounded-3xl bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/5 backdrop-blur-xl mb-8 relative overflow-hidden shadow-sm dark:shadow-none">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <AnimatePresence mode="wait">
              {orderData.status === 'payment_in_escrow' && (
                <div key="escrow" className="p-6 rounded-3xl bg-blue-500/10 border border-blue-500/20 backdrop-blur-sm">
              ...
              {orderData.status === 'shipped' && (
                <div key="shipped" className="p-6 rounded-3xl bg-purple-500/10 border border-purple-500/20 backdrop-blur-sm relative overflow-hidden">
              ...
              {orderData.status === 'courier_verified' && (
                <div key="courier_verified" className="p-6 rounded-3xl bg-indigo-500/10 border border-indigo-500/20 backdrop-blur-sm">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-zinc-600 dark:text-zinc-400 text-sm mb-4 leading-relaxed">
                        Package verified by courier. On route to destination.
                      </p>
              ...
              {orderData.status === 'inspection' && (
                <div key="inspection" className="p-6 rounded-3xl bg-orange-500/10 border border-orange-500/20 backdrop-blur-sm">
                  <div className="flex items-start gap-4">
              ...
              {orderData.status === 'completed' && (
                <div key="completed" className="p-6 rounded-3xl bg-green-500/10 border border-green-500/20 backdrop-blur-sm">
                  <div className="flex items-start gap-4">
              ...
              {orderData.status === 'disputed' && (
                <div key="disputed" className="p-6 rounded-3xl bg-red-500/10 border border-red-500/20 backdrop-blur-sm">
                  <div className="flex items-start gap-4">
          ...
          <div className="lg:col-span-1">
            <div className="p-6 rounded-3xl bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/5 backdrop-blur-xl sticky top-24 shadow-sm dark:shadow-none">
